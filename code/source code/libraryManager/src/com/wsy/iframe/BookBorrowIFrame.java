...
public class BookBorrowIFrame extends JInternalFrame {
	...
	private String[] columnNames = { "ISBN", "Borrowing Date", "Due Date", "Reader Number"  };
...
//		});//关闭窗口时候引发的事件
		...
		setTitle("Book Borrowing Management");
		...
		buttonBorrow.setText("Lend the book");
				...
				if (readerISBN.getText().trim().length()!=0
					...
					if (!readerISBNs.isEmpty() && list5.isEmpty()) {
						JOptionPane.showMessageDialog(null,
								"Not registed！Please Check!");
						return;
					}
					if (list4.isEmpty() && !ISBNs.isEmpty()) {
						JOptionPane.showMessageDialog(null,
								"No that book！Please Check!");
						return;
					}
					if (Integer.parseInt(number.getText().trim()) <= 0) {
						JOptionPane.showMessageDialog(null, "The book is lent！");
						return;
					}
		...
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				...
				if (list.isEmpty() && !ISBNs.isEmpty()) {
					JOptionPane.showMessageDialog(null,
							"The Reader no. is Not registed！Please Check!");
				}
				for (int i = 0; i < list.size(); i++) {
					...
					System.out.println("You can borrow" + number.getText().trim());
				}
			}
		}
	}
	...
	class BorrowActionListener implements ActionListener { 
		public void actionPerformed(final ActionEvent e) {
			...
			if(i==1){
				JOptionPane.showMessageDialog(null, "Borrow finished！");
	...
	class ClearActionListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if(table.getRowCount()!=0){
				...
				JOptionPane.showMessageDialog(null, "There is no data in the table now. Please borrow it");
			}
		}
	}
