...
public class ReaderModiAndDelIFrame extends JInternalFrame {
	...
	private String[] columnNames={ "ReaderName", "ReaderSexual", "ReaderAge", "IDcardNo.", "Validate Date",
			"MaxBorrowed", "Tel","Deposit","Certi.","Career","ReaderNo.","CreatedTime" };
	private String[] array=new String[]{"ID card","Student Card","Citizen Card","Stuff Card"};
	String id;
	...
	public ReaderModiAndDelIFrame() {
		...
		setIconifiable(true);
		setClosable(true);
		setTitle("Readerinfo. modify or delete");
		...
		ImageIcon readerModiAndDelIcon=CreatecdIcon.add("LMS_ModiDelReader.jpg");
		...
		panel.add(logoLabel);
		logoLabel.setText("Readerlogo（400*80）");
		...
		label.setText("  Validate ID card：");
		panel_2.add(label);
		...
		label_9.setText(" Max borrowed number：");
		panel_2.add(label_9);
		...
		label_13.setText(" Validate date：");
	...
	class TableListener extends MouseAdapter {
		public void mouseClicked(final MouseEvent e) {
			...
			if(table.getValueAt(selRow, 1).toString().trim().equals("Male"))
				JRadioButton1.setSelected(true);
			else
	...
	class ModiButtonListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if(age.getText().length()==0){
				JOptionPane.showMessageDialog(null, "Reader Age cannot be null.");
				return;
			}
			...
			if(ISBN.getText().length()==0){
				JOptionPane.showMessageDialog(null, "Reader barcode cannot be null.");
				return;
			}
			if(ISBN.getText().length()!=13){
				JOptionPane.showMessageDialog(null, "Reader barcode is 13 dits");
				return;
			}
			if(bztime.getText().length()==0){
				JOptionPane.showMessageDialog(null, "Created time cannot be null.");
				return;
			}
			...
			if(tel.getText().length()>11||tel.getText().length()<0){
				JOptionPane.showMessageDialog(null, "Tel number is 11 bits");
				return;
			}
			...
			if(maxnumber.getText().length()>2||tel.getText().length()<0){
				JOptionPane.showMessageDialog(null, "Max borrow number is up to 2bits.");
				return;
			}
			String sex="1";
